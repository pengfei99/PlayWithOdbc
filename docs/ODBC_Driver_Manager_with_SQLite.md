# Installing the ODBC Driver Manager on Linux

The objective of this doc is to document how to set a self-standing environment for testing an ODBC driver of a
DBMS. 

## 1. Installing the ODBC driver manager

There are 2 main implementations of ODBC for Linux: 
- UnixODBC (http://www.unixodbc.org/)
- iODBC (http://www.iodbc.org); 

In this tutorial, we picked the former for no particular reason.

The driver manager can be installed as root through the standard package management tool in Debian:

```shell
# This package provides the shared libraries required for applications using the ODBC (Open Database Connectivity) API. 
# It contains the runtime components necessary for ODBC connectivity and allows applications to connect to various 
# databases through ODBC drivers.
sudo apt-get install libodbc1

# This package provides an implementation of the ODBC API for unix like OS. It contains various tools,
# libs, and drivers
# In this tutorial, we will the ODBC driver manager provided by this package
sudo apt install unixodbc

# check the installed version
odbcinst --version

# output unixODBC 2.3.11

```

**unixodbc** provide a CLI, you can use it to do many things
...
## 2. Install the SQLite ODBC driver

### 2.1. Install sqlite and odbc driver

First, we install the SQLite binary along with its administrative tool isql are installed by the native package manager
...
Each time an ODBC driver is installed, it updates the file **/etc/odbcinst.ini** with driver-dependent information.
You can check the currently installed driver with the command `odbcinst -q -d`

### 2.2 Configure sqlite odbc data source

For now, we only change the current user config which is located at `~/.odbc.ini`. The system-wide config file is located at `/etc/.odbc.ini`

Open it and do the below config
...
Driver=SQLite3
Database=/home/pengfei/data_set/odbc/sqlite/sample.db 
```

Now let's check the newly added DSN

```shell
# show all available DSN
odbcinst -q -s
...
# now show the details of the mysqlitedb DSN
odbcinst -q -s -n mysqlitedb
```

The **.odbc.ini** file allows us to hide a drivers’ `private settings` so that we don’t need to specify them later. 
When connecting to a data source, we can use the name that is specified here between square brackets (e.g. [mysqlitedb]), 
which is the DSN. It is also possible to directly specify in-line all the parameters in this section when connecting 
programmatically but the code will need to be edited (and maybe recompiled) in case one them has to be changed, 
so using a DSN is preferable.

> The default file location is the current user’s home directory, but it can be changed and its full path name set in 
  the environment variable $ODBCINI.


### 2.3 Init a sqliteDB table 

```shell
# start the sqlite CLI
sqlite3

# you should see below CLI
...
# open a persistent database. Here we use the /home/pengfei/data_set/odbc/sqlite/sample.db 
.open /home/pengfei/data_set/odbc/sqlite/sample.db 
```

Use below sql file to create tables and schema
```sql
...
Use below sql command to populate the tables
...
Run below command to test the database
...
# If the above command returned the below output, it means everything is setup correctly
China|CN|||
...
Type **.quit** or use **Ctrl+D** to exit the terminal.

## 3. Test the sqlite odbc DSN with isql 

The `isql` tool is a command-line utility provided by the unixODBC package. It allows you to connect to a database, 
execute SQL queries or commands, and retrieve the results `via ODBC connectivity`.

```shell
# connect to a odbc DSN
isql -v <DSN> <userId> <pwd>

# in our case, we can just run
isql -v mysqlitedb

# if you see below error, run the below export to include lib binary
# [01000][unixODBC][Driver Manager]Can't open lib 'libsqlite3odbc.so' : file not found
# [ISQL]ERROR: Could not SQLConnect

# Most of the ODBC drivers have their shared libraries installed in /usr/lib/x86_64-linux-gnu/odbc, 
# this path must be added the $LD_LIBRARY_PATH environment variable
export LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu/odbc:$LD_LIBRARY_PATH

```

Run below sql command and check the result
...
If it returns the same result as the previous section. It means your odbc connection works correctly

## 4. Test the sqlite odbc DSN with pyodbc

We can also use a python client to connect to the odbc DSN.
...
python odbc_manager.py

# you should see below output
('China', 'CN', None, None, None)
('United Kingdom', 'UK', None, '8204 Arthur St', 'London')
('United Kingdom', 'UK', None, 'Magdalen Centre, The Oxford Science Park', 'Oxford')
('United States of America', 'US', None, '2014 Jabberwocky Rd', 'Southlake')
('United States of America', 'US', None, '2011 Interiors Blvd', 'South San Francisco')
('United States of America', 'US', None, '2004 Charade Rd', 'Seattle')

```

The python code source can be found [here](../src/odbc_manager.py)
